...
public class AprilTagStats extends SubsystemBase {
        ...
        if (m_arduCam.isConnected()) {
            ...
            if (result.hasTargets()) {
                ...
                if (m_layout.getTagPose(target.getFiducialId()).isPresent()) {
                    relativePose = getRobotPose(transform, target, m_robotToCam);
    ...
    public double getDistance() {
        ...
        if (relativePose != null && target != null) {
            double tagPitch = target.getPitch();
            double hypotenuse = PhotonUtils.getDistanceToPose(relativePose.toPose2d(), m_layout.getTagPose(target.getFiducialId()).get().toPose2d());
